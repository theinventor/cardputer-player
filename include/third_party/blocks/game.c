...
void game_shape(int kind, int output[8]) {
  if (kind >= 0 && kind < N_SHAPES) memcpy(output, SHAPES[kind], sizeof(SHAPES[kind]));
}
...
int get_next_shape_kind(game_state_t *state) {
  state->shape_bag_idx = (state->shape_bag_idx + 1) % N_SHAPES;

  if (state->shape_bag_idx == 0) {
    ...
    for (int i = N_SHAPES - 1; i > 0; i--) {
      int j = rand() % (i + 1);
      int temp = state->shape_bag[i];
      state->shape_bag[i] = state->shape_bag[j];
      state->shape_bag[j] = temp;
    }
  }

  return state->shape_bag[state->shape_bag_idx];
}

void spawn_shape(game_state_t *state) {
  state->changed = 1;

  state->current_shape_kind = state->next_shape_kind;
  state->next_shape_kind = get_next_shape_kind(state);
...
void restart_game(game_state_t *state) {
  ...
  state->shape_bag_idx = N_SHAPES - 1;
  state->next_shape_kind = get_next_shape_kind(state);
...
void clean_destroyed_blocks(game_state_t *state) {
  ...
  for (int r = 0; r < GRID_HEIGHT; r++) {
    if (state->rows_to_destroy & (1 << r)) {
      count++;

      state->rows_to_destroy ^= (1 << r);
      for (int c = 0; c < GRID_WIDTH; c++) {
        state->grid[r][c] = 0;
      }
      destroy_row(state, r);
