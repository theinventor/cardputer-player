...
export function createPreviewServer({pageSize=8}={}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    requests.push({path:url.pathname, method:req.method, params:Object.fromEntries(form), query:Object.fromEntries(url.searchParams)});
    ...
    if(url.pathname === '/api/library') {
      const q=(url.searchParams.get('q')||'').toLowerCase(),offset=Number(url.searchParams.get('offset')||0),requested=Number(url.searchParams.get('limit')||16);
      if(!Number.isInteger(offset)||offset<0||!Number.isInteger(requested)||requested<1||requested>64){send(400,{error:'Invalid page'});return;}
      const limit=Math.min(requested,pageSize),matches=tracks.filter(t=>t.id>=offset&&JSON.stringify(t).toLowerCase().includes(q));
      send(200,{tracks:matches.slice(0,limit),total:tracks.length,next_offset:matches[limit]?.id??-1});return;
    }
    if(url.pathname === '/api/queue') {
      const offset=Number(url.searchParams.get('offset')||0);
      if(!Number.isInteger(offset)||offset<0||offset>64){send(400,{error:'Invalid queue page'});return;}
      const end=Math.min(queue.length,offset+pageSize);
      send(200,{tracks:queue.slice(offset,end).map(id=>tracks[id]),total:queue.length,next_offset:end<queue.length?end:-1});return;
    }
    ...
    if(url.pathname === '/api/control') {
      ...
      if(action === 'playlist') {
        ...
        state.playlist_position=form.has('entry')?Number(form.get('entry')):p?p.ids.indexOf(state.track.id):state.track.id;
        if(form.has('entry'))state.track=tracks[p?p.ids[state.playlist_position]:state.playlist_position];
      ...
      else if(action === 'play-entry'){state.playlist_position=Number(value);state.track=tracks[lists.get(state.active_playlist_id).ids[Number(value)]];state.state='playing';}
        ...
        if(queue.length>=64||(state.active_playlist_id&&!lists.get(state.active_playlist_id)?.ids.includes(Number(value)))){send(400,{error:'Queue full or track outside the active playlist'});return;}
  ...
  return {server,lists,tracks,state,requests,failNext(message){failNext=message;},setQueue(ids){queue=[...ids];state.queue_count=queue.length;}};
}
